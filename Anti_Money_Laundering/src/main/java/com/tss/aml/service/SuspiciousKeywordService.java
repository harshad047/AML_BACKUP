...
public class SuspiciousKeywordService {
    ...
    public int calculateRiskScore(String text) {
        ...
        for (SuspiciousKeyword keyword : activeKeywords) {
            if (containsKeyword(processedText, keyword)) {
                totalScore += keyword.getRiskScore();
                System.out.println("Keyword found: '" + keyword.getKeyword() + "' (+" + keyword.getRiskScore() + " points, " + keyword.getRiskLevel() + ")");
            }
        }
