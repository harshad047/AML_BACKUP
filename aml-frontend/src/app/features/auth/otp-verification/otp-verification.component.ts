          ...
          <div class="card shadow-lg">
            <div class="card-body p-5">
              <div class="text-center mb-4">
                <h2 class="fw-bold text-primary">{{ isRegistration ? 'Complete Registration' : 'Verify OTP' }}</h2>
                <p class="text-muted">{{ isRegistration ? 'Enter the verification code sent to complete your registration' : 'Enter the verification code sent to your email' }}</p>
                <small class="text-muted">{{ email }}</small>
                <div *ngIf="userName" class="mt-2">
              ...
              <form [formGroup]="otpForm" (ngSubmit)="onSubmit()">
                ...
                <div class="d-grid">
                  ...
                  >
                    ...
                    {{ isLoading ? 'Verifying...' : (isRegistration ? 'Complete Registration' : 'Verify & Login') }}
                  </button>
                </div>
                ...
                <div class="text-center mt-2">
                  <button
                    ...
                    (click)="goBackToLogin()"
                  >
                    {{ isRegistration ? 'Back to Registration' : 'Back to Login' }}
                  </button>
                </div>
              </form>
            </div>
...
export class OtpVerificationComponent implements OnInit, OnDestroy {
  ...
  email = '';
  userName = '';
  isRegistration = false; // Flag to determine if this is registration or login OTP
  ...
  ngOnInit(): void {
    ...
    // Get email and flow type from route params
    this.route.queryParams.subscribe(params => {
      this.email = params['email'] || '';
      this.userName = params['name'] || '';
      this.isRegistration = params['type'] === 'registration' || !!params['name'];
    });
    ...
    if (this.otpForm.valid) {
      ...
      if (this.isRegistration) {
        ...
        this.authService.verifyOtp(this.email, otp).subscribe({
          ...
          error: (error) => {
            this.isLoading = false;
            this.errorMessage = error.error?.error || error.error?.message || 'Verification failed. Please try again.';
  ...
  resendOtp(): void {
    ...
    const otpObservable = this.isRegistration 
